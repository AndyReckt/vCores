            ...
            if (!sender.hasPermission(plugin.getConfig().getString("Grants.Permission"))) {
                ...
                if (args.length != 1) {
                    ...
                    if (target != null) {
                        ...
                        Inventory grantsInv = Bukkit.createInventory(player, 54, ChatColor.DARK_GRAY + "Grants of: " + target.getDisplayName());
                        ...
                        if (plugin.gData.config.contains(target.getUniqueId() + ".Grants")) {
                            GrantManagement grantManagement = new GrantManagement(target);
                            for (String userGrants : plugin.gData.config.getConfigurationSection(target.getUniqueId().toString() + ".Grants").getKeys(false)) {
                                Grants++;
                                String exeUuid = plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Executor");
                                String dfColor = "";
                                ...
                                reason = plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Reason");
                                grant = plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Rank");
                                ...
                                server = plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Server");

                                ItemStack grantItem = XMaterial.LIME_WOOL.parseItem();
                                ItemMeta grantMeta = grantItem.getItemMeta();

                                if (plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Status").equalsIgnoreCase("Active")) {
                                ...
                                } else if (plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Status").equalsIgnoreCase("Revoked")) {
                                ...
                                } else if (plugin.gData.config.getString(target.getUniqueId().toString() + ".Grants." + Grants + ".Status").equalsIgnoreCase("Expired")) {
