    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
        if (!(sender instanceof Player)) {
            ...
            if (!sender.hasPermission(plugin.getConfig().getString("AdminChat.permission"))) {
                ...
                if (args.length == 0) {
                    Utils.sendMessage(player, plugin.getConfig().getString("AdminChat.incorrectusage").replaceAll("%plugin_prefix%", plugin.getConfig().getString("PluginPrefix")).replaceAll("%server_prefix%", plugin.getConfig().getString("ServerPrefix")));
                }
                    ...
                    for (Player staff : Bukkit.getOnlinePlayers()) {
                        if (staff.hasPermission(plugin.getConfig().getString("AdminChat.permission"))) {
                            staff.sendMessage(ChatColor.translateAlternateColorCodes('&', plugin.getConfig().getString("AdminChat.format").replaceAll("%world%", world).replaceAll("%player%", player.getDisplayName()).replaceAll("%message%", message)));
                        }
                    }
                }
            }
