    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (!sender.hasPermission(plugin.getConfig().getString("Alts.Permission"))) {
            ...
            if (sender instanceof Player) {
                ...
                if (args.length != 1) {
                    Utils.sendMessage(sender, plugin.getConfig().getString("Alts.IncorrectUsage").replaceAll("%server_prefix%", plugin.getConfig().getString("ServerPrefix")).replaceAll("%plugin_prefix%", plugin.getConfig().getString("PluginPrefix")));
                } else {
                    OfflinePlayer target2 = Bukkit.getOfflinePlayer(args[0]);
                    int amount = 0;
                    for (String allUUIDS : plugin.data.config.getConfigurationSection("IPs").getKeys(false)) {
                    ...
                    if (amount >= 1) {
                        ...
                        Utils.sendMessage(sender, plugin.getConfig().getString("Alts.CommandMessage").replaceAll("%player%", target2display).replaceAll("%alts%", str));
                        plugin.alts.clear();
                    }
                }
            }
